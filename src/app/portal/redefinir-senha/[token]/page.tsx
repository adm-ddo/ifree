...
import RedefinirSenhaPessoaForm from "./RedefinirSenhaPessoaForm";
...
export default async function RedefinirSenhaPessoaPage({
  params,
}: {
    ...
    return (
      <div className="flex flex-1 items-center justify-center py-8">
        <div className="flex flex-col gap-3 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm w-full max-w-md">
          ...
          <Link href="/portal/esqueci-senha" className="text-brand-700 underline text-sm">
            Pedir novo link
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-1 items-center justify-center py-8">
      <RedefinirSenhaPessoaForm token={token} />
    </div>
  );
}
