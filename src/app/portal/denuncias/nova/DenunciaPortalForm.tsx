...
import { criarDenunciaPortal } from "./actions";
import { CATEGORIAS_DENUNCIA } from "@/lib/etica-constantes";

export default function DenunciaPortalForm({
  empresas,
}: {
  empresas: { id: number; nome: string }[];
}) {
  const [state, formAction, pending] = useActionState(criarDenunciaPortal, undefined);
  const [identificarSe, setIdentificarSe] = useState(true);
  ...
  if (state?.protocolo && state?.senha) {
    return (
      <div className="rounded-2xl border-2 border-brand-300 bg-brand-50 p-6 shadow-sm flex flex-col gap-4">
        <h2 className="text-lg font-semibold text-navy-900">Denúncia registrada (anônima)</h2>
        <p className="text-sm text-stone-700">
          Como você optou por anonimato, esta denúncia <strong>não</strong>{" "}
          aparece em &ldquo;Minhas denúncias&rdquo; — anote o protocolo e a
          senha abaixo, não vão aparecer de novo.
        </p>
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4 rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
      <div>
        <label className="text-sm font-medium text-navy-900 block mb-1">Empresa</label>
        <select
          name="empresaId"
          required
          defaultValue=""
      ...
      <label className="flex items-start gap-2 rounded-lg border border-stone-200 px-3 py-2.5">
        <input
          type="checkbox"
          name="identificarSe"
        ...
        />
        <span>
          <span className="block text-sm font-medium text-navy-900">Identificar-me com meu perfil</span>
          <span className="block text-xs text-stone-500">
            {identificarSe
              ? "A empresa vê seu nome e a denúncia fica em “Minhas denúncias”."
              : "Desmarcado: sua denúncia fica totalmente anônima, sem nenhum vínculo com seu perfil — você acompanha só com protocolo+senha."}
          </span>
        </span>
      </label>
      ...
      >
        {pending ? "Enviando..." : "Registrar denúncia"}
      </button>
    </form>
  );
}
