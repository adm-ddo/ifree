...
import { gerarPdfRelatorioFuncao } from "@/lib/relatorio-funcao-pdf";
...
export async function GET(req: Request) {
  ...
  const pdfBytes = await gerarPdfRelatorioFuncao({
  ...
  return new NextResponse(new Uint8Array(pdfBytes), {
    headers: {
      ...
      "Content-Disposition": `inline; filename="custo-por-funcao-${dataISOBrasil(dataInicio)}.pdf"`,
